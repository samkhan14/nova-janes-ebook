@props([
    'instagram' => 'https://www.instagram.com/author_jane_mansons?igsh=MW9qNHd0YzE1cWI4aw==',
    'facebook' => 'https://www.facebook.com/share/1D4edKsnNz/',
    'threads' => 'https://www.threads.net/@authorjanemansons',
    'variant' => 'header',
])

<div {{ $attributes->class(['social-links', 'social-links--'.$variant]) }} aria-label="Social links">
    @if (filled($instagram))
        <a class="social-links__link" href="{{ $instagram }}" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
    ...
    @endif
    @if (filled($facebook))
        <a class="social-links__link" href="{{ $facebook }}" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
            <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
    ...
    @endif
    @if (filled($threads))
        <a class="social-links__link" href="{{ $threads }}" target="_blank" rel="noopener noreferrer" aria-label="Threads">
            <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
